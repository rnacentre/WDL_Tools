...
// src/node/plugins/markdown-title.ts
var REG_HEADING = /^#\s*?([^#\s].*)?\n/;
function markdownTitlePlugin() {
  return {
...
async function prepareArticleTagColors(app, localeOptions) {
  const start = performance.now();
  const blog = isPlainObject3(localeOptions.blog) ? localeOptions.blog : {};
...
import { entries as entries5, isLinkAbsolute as isLinkAbsolute2, isLinkHttp, isPlainObject as isPlainObject4 } from "@vuepress/helper";
...
async function prepareIcons(app, localeOptions) {
  ...
  uniq3(iconList).filter((icon) => {
    if (fsCache2?.data?.[icon] && !cache2[icon])
      cache2[icon] = fsCache2.data[icon];
    return !cache2[icon];
  }).forEach((iconName) => {
  ...
  let cssCode = "";
  const map = {};
  for (const [iconName, { className, content, background }] of entries5(cache2)) {
...
function getIconWithSidebar(sidebar) {
  ...
  } else if (isPlainObject4(sidebar)) {
...
function getSidebarData(app, locales) {
  ...
  entries6(locales).forEach(([localePath, sidebar]) => {
    ...
    } else if (isPlainObject5(sidebar)) {
...
import {
  ensureLeadingSlash as ensureLeadingSlash3,
  getRootLang,
...
function autoCategory(page, options) {
  ...
  const categoryList = list.map((category, index) => {
    ...
    if (!cache3[match[2]] && !match[1]) {
      cache3[match[2]] = uuid++;
    }
    return {
      id: hash2(list.slice(0, index + 1).join("-")).slice(0, 6),
      sort: Number(match[1] || cache3[match[2]]),
      name: match[2]
    };
  });
...
function enableBulletin(page, options) {
  let enablePage;
  if (isPlainObject6(options.bulletin) && options.bulletin.enablePage) {
    enablePage = options.bulletin.enablePage;
  } else if (options.locales) {
    for (const locale of Object.keys(options.locales)) {
      if (isPlainObject6(options.locales[locale].bulletin) && options.locales[locale].bulletin.enablePage) {
  ...
  return (app) => {
    ...
    return {
      ...
      extendsPage: async (page) => {
        const { localeOptions: localeOptions2 } = getThemeConfig();
